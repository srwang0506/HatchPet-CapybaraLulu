...
- 🌿 **静止时也有生命力。** 呼吸、眨眼、张嘴、单手挥动与自然回正被编排在同一条连续时间线上。
- 💻 **看得懂任务状态。** 工作中、等待输入、完成待查看和失败/阻塞都有独立而清晰的反应。
- 🐾 **真正参与桌面互动。** 鼠标悬停会跳跃，左右拖动时会用交替步态跟着移动。
- 👀 **会跟着你看。** 16 个顺时针注视姿态以 22.5° 为间隔覆盖完整指针方向。
- 🎞️ **幕后素材完整公开。** 83 张透明 PNG、全部 GIF、构建工具与验证器均包含在仓库中。

## 🎬 动作图鉴

每个动作都保持原本的播放速度。下面的无边框动作卡片同时展示动态预览、Codex 状态名、触发条件以及对应的全部源帧。

<p>
  <img src="assets/gifs/idle.gif" alt="噜噜灵动静止循环" width="124" align="left">
  <strong>🌿 灵动静止</strong><br>
  <sub><code>idle</code> · 16 帧 · 2.13 秒</sub><br><br>
  没有活动任务状态、且鼠标位于中性死区时出现。WebP 时间轴会连续播放呼吸、眨眼、张嘴、单手挥动与自然回正。<br>
  <a href="assets/frames/idle/">查看全部 16 张源帧 →</a>
</p>
<br clear="left">
...
<p>
  ...
  <sub><code>running-right</code> · 8 帧 · 1.06 秒</sub><br><br>
  向屏幕右侧拖动悬浮桌宠时出现。<br>
  <a href="assets/frames/running-right/">查看全部 8 张源帧 →</a>
</p>
<br clear="left">
...
<p>
  ...
  <sub><code>running-left</code> · 8 帧 · 1.06 秒</sub><br><br>
  向屏幕左侧拖动悬浮桌宠时出现。<br>
  <a href="assets/frames/running-left/">查看全部 8 张源帧 →</a>
</p>
<br clear="left">
...
<p>
  ...
  <sub><code>waving</code> · 4 帧 · 0.70 秒</sub><br><br>
  唤醒噜噜后，作为首次醒来问候出现。<br>
  <a href="assets/frames/waving/">查看全部 4 张源帧 →</a>
</p>
<br clear="left">
...
<p>
  ...
  <strong>✨ 跳跃</strong><br>
  <sub><code>jumping</code> · 5 帧 · 0.84 秒</sub><br><br>
  鼠标进入或悬停在噜噜上方时出现。<br>
  <a href="assets/frames/jumping/">查看全部 5 张源帧 →</a>
</p>
<br clear="left">
...
<p>
  ...
  <sub><code>failed</code> · 8 帧 · 1.22 秒</sub><br><br>
  对话失败、任务被阻塞或遇到系统错误时出现。<br>
  <a href="assets/frames/failed/">查看全部 8 张源帧 →</a>
</p>
<br clear="left">
...
<p>
  ...
  <sub><code>waiting</code> · 6 帧 · 1.01 秒</sub><br><br>
  Codex 需要审批、回答或其他用户决定时出现。<br>
  <a href="assets/frames/waiting/">查看全部 6 张源帧 →</a>
</p>
<br clear="left">
...
<p>
  ...
  <sub><code>running</code> · 6 帧 · 0.82 秒</sub><br><br>
  对话正在工作时出现。噜噜会拿电脑处理任务，而不是原地跑步。<br>
  <a href="assets/frames/running/">查看全部 6 张源帧 →</a>
</p>
<br clear="left">
...
<p>
  ...
  <sub><code>review</code> · 6 帧 · 1.03 秒</sub><br><br>
  对话已经完成、且存在尚未查看的活动时出现。<br>
  <a href="assets/frames/review/">查看全部 6 张源帧 →</a>
</p>
<br clear="left">
...
- 🟢 **ChatGPT 桌面端 · Codex——原生悬浮桌宠。** 运行 `python3 scripts/install.py`，重启应用，然后在 **设置 → Pets** 中选择噜噜。
- 🟠 **Codex CLI——在受支持终端中原生显示。** 安装同一本地宠物包，再在 iTerm2 3.6+、Kitty 图形协议或支持 Sixel 的终端中通过 `/pets` 选择。
- ⚪ **Codex IDE 扩展——没有桌宠界面。** 需要悬浮桌宠时使用桌面端或 Codex CLI。
- 🔵 **Claude Code——可维护项目，但没有 Codex 桌宠渲染器。** 它可以编辑仓库、源帧和文档；把 `pet/` 复制到 `~/.claude` 不会出现悬浮桌宠。
