...
实际安装的 `pet/spritesheet.webp` 是一张带时间轴的 8 × 11 动态图集。每个原生状态都有 20 个同步时间相位，共享时钟仍为 1.60 秒；左右跑会在其中完成两轮各 0.80 秒的完整步态，因此跑动更利落，却不会连带加快静止、挥手或玩电脑。15 个具名视觉动作片段被分配到 9 个真实 Codex 触发状态中；仓库同时保留静态图集，供质量检查、二次编辑和减少动态效果时使用。

<p align="center">
  <img src="assets/gifs/idle.gif" alt="噜噜呼吸、眨眼与挥手" width="132">
  <img src="assets/gifs/running-left.gif" alt="噜噜向屏幕左侧跑动" width="132">
  <img src="assets/gifs/running-right.gif" alt="噜噜向屏幕右侧跑动" width="132">
  <img src="assets/gifs/running.gif" alt="噜噜拿电脑工作" width="132">
  <img src="assets/gifs/look-directions.gif" alt="噜噜跟随鼠标注视" width="132">
</p>

<p align="center"><sub>日常陪伴 · 左右移动 · 陪 Codex 工作 · 跟随鼠标注视</sub></p>

| 噜噜的能力 | 仓库中实际提供的内容 |
| :--- | :--- |
| 🌿 **灵动静止** | 20 个时间相位：休息、呼吸、眨眼、张嘴与闭嘴、单手挥动，以及柔和的中性回归。 |
| 💻 **任务感知** | 工作中、等待输入、完成待查看与失败/阻塞各有独立反应；仅工作状态就包含敲键、眨眼、低头读屏和另一组交替敲键四个微循环。 |
| 🐾 **桌面互动** | 鼠标悬停时跳跃；向左或向右拖动时，以两轮各 0.80 秒的完整步态连续跑动。 |
...
<p>
  ...
  <sub><code>running-right</code> · 20 个独立相位 · 2 × 0.80 秒</sub><br><br>
  向屏幕右侧拖动悬浮桌宠时出现。两轮不同的完整步态会交替同色双腿与相反方向摆动的手臂，不再出现像尾巴一样的后侧突起。<br>
...
<p>
  ...
  <sub><code>running-left</code> · 20 个独立相位 · 2 × 0.80 秒</sub><br><br>
  向屏幕左侧拖动悬浮桌宠时出现。它由通过检查的右跑步态逐帧镜像而来，帧序与时间完全一致。<br>
...
两个方向都使用 20 个独立姿势，并拆成两轮各 10 相位的完整步态；每轮在共享的 1.60 秒图像时间轴中用时 0.80 秒。左跑逐帧镜像已通过检查的右跑，因此两边的节奏、身体锚点与落脚顺序完全对应。

<p align="center">
  <img src="assets/directional-gait.png" alt="水豚噜噜向右与向左跑步的 20 相位动作分解" width="100%">
</p>

<p align="center">
  <img src="assets/gifs/running-left.gif" alt="水豚噜噜最终向左跑步循环" width="190">
  <img src="assets/gifs/running-right.gif" alt="水豚噜噜最终向右跑步循环" width="190">
</p>

<p align="center"><sub>两轮 0.80 秒步态 · 同一套 20 相位时钟 · 严格逐帧镜像</sub></p>
...
- 左右跑都包含 20 个独立姿势并组成两轮 0.80 秒完整步态，左跑是已通过检查的右跑逐帧镜像，且相位顺序保持不变；
- 两条跑步腿必须使用同一套黄色绒面配色，不保留深色的腿部识别标记；
