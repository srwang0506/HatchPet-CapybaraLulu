---
name: hatch-pet
description: Create, repair, validate, visually QA, and package Codex-compatible v2 animated pets from character art, generated images, company or prospect brand cues, or visual references. Use for any new Codex pet, custom mascot, non-pixel pet style, brand-inspired pet, existing-pet repair, desktop idle-stutter or choppy-blink repair, smooth image-time WebP runtime packaging, 12-15 visual-motion libraries mapped to the 9 native triggers, or 8x11 spritesheet workflow requiring all 9 standard animation rows, 16 look directions, deterministic assembly, QA artifacts, and spriteVersionNumber 2 packaging.
---
...
### Optional Smooth-Idle Runtime Package

If the user explicitly requested desktop idle smoothing, follow `references/desktop-smooth-idle.md` after the static v2 atlas passes every normal gate. Package `final/spritesheet-runtime.webp` with `package_smooth_idle_webp.py`, run both `validate_atlas.py --require-v2 --allow-animated --allow-transparent-rgb-residue` and `validate_smooth_idle_webp.py`, and inspect the exact loop rendered by `render_smooth_idle_preview.py` before installing that runtime file as `spritesheet.webp`. For an expressive idle, pass the same reviewed `--phase-manifest` to both packaging and specialized validation.

If the user requested synchronized high-phase motion across non-idle states, follow `references/desktop-smooth-state.md` instead of layering two animated wrappers. Package with `package_smooth_state_webp.py --require-all-states`, validate with `validate_smooth_state_webp.py --require-all-states`, render and visually inspect every native state, and confirm that the manifest's declared motion clips are visibly present. Keep the static `final/spritesheet-extended.webp` as the QA master in either path. Install under a new id or fully restart Codex so its process-level custom-pet cache cannot retain the prior file.
...
- The smooth image-time WebP wrappers are the only deterministic tiling exception. Smooth idle may duplicate one approved phase cell across the six selectable idle columns. Smooth state may duplicate one approved state-phase cell across every renderer-selectable column of that same native row. Both exceptions apply only after the complete static v2 atlas passes QA. A custom phase cell must be an approved `192x208` visual generated through `$imagegen` or an approved populated cell from the static atlas and must be declared in a phase manifest. A wrapper may not synthesize, interpolate, redraw, replace, or satisfy a missing visual frame.
...
- When smooth-idle runtime packaging uses the default plan, the animated WebP has exactly six frames at `280,110,110,140,140,320` ms. When an expressive-idle phase manifest is used, frame count, order, labels, source cells, and positive durations match that manifest exactly. In both modes every frame keeps idle columns `0-5` synchronized, preserves row-0 neutral and unused cells, keeps rows `1-10` at exact source alpha and visible RGB, passes `validate_atlas.py --require-v2 --allow-animated --allow-transparent-rgb-residue` on frame 0, and passes `validate_smooth_idle_webp.py` for the complete file. Visual preview additionally confirms that a declared one-limb gesture never switches sides, moves the opposite limb, or jumps between sparse distant positions.
- When smooth-state runtime packaging is selected, every configured native row has exactly the manifest's global phase count and durations, all renderer-selectable columns in that row are phase-synchronized, look rows remain exact, and `validate_smooth_state_webp.py` passes the complete file. A request for “about 20 frames” normally means 16-24 approved image-time phases. Report the count of unique approved poses separately from repeated timing cells, and report 12-15 named visual clips separately from the fixed count of nine native triggers.
