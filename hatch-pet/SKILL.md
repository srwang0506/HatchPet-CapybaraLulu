---
name: hatch-pet
description: Create, repair, validate, visually QA, and package Codex-compatible v2 animated pets from character art, generated images, company or prospect brand cues, or visual references. Use for any new Codex pet, custom mascot, non-pixel pet style, brand-inspired pet, existing-pet repair, desktop idle-stutter or choppy-blink repair, smooth-idle animated WebP runtime packaging, or 8x11 spritesheet workflow requiring all 9 standard animation rows, 16 look directions, deterministic assembly, QA artifacts, and spriteVersionNumber 2 packaging.
---
...
- The smooth-idle animated WebP wrapper is the only deterministic tiling exception: it may duplicate one approved phase cell across the six selectable idle columns of one WebP time frame, solely after the complete static v2 atlas passes QA. A custom phase cell must be an approved `192x208` visual generated through `$imagegen` or an approved populated cell from the static atlas and must be declared in a phase manifest. The wrapper may not synthesize, interpolate, redraw, replace, or satisfy a missing visual frame.
