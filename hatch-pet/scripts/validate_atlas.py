...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="warn instead of fail for codec-normalized hidden RGB under alpha 0",
    )
    parser.add_argument(
        "--allow-animated",
        action="store_true",
    ...
    try:
        with Image.open(atlas_path) as opened:
            ...
            source_is_animated = bool(getattr(opened, "is_animated", False))
            source_frame_count = int(getattr(opened, "n_frames", 1))
            opened.seek(0)
    ...
    if transparent_rgb_residue:
        ...
        if args.allow_transparent_rgb_residue:
            warnings.append(message)
        else:
            errors.append(message)
